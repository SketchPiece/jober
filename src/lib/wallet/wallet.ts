...
import createServiceError from '$lib/helpers/createServiceError';
...
const SUBMIT_BUTTON_SELECTOR = '#root > div > div > section > div > form > button';
...
export class WalletClient {
		...
		try {
			...
			const accounts = await page.evaluate((ACCOUNTS_SELECTOR) => {
				...
				for (let i = 0; i < accountItems.children.length; i++) {
					...
					const accountName = item.querySelector('._2N3zjkUa9I6_GTkadL9ZDa')?.textContent;
					const accountType = item.querySelector('._2KYPmp9YZ-gBS_2ynKzzcE')?.textContent;
					const rawAccountBalance =
						item.querySelector('._1_RYaZbAcU6dw5pzNHpN56')?.textContent || '0';
					const accountBalance = parseFloat(rawAccountBalance.replace(/[^0-9.-]+/g, ''));
					accounts.push({
						name: accountName || '',
						type: accountType || '',
	...
	async addRecord(accountName: string, amount: number, type: 'expense' | 'income'): Promise<void> {
		try {
			const page = await this.getPage();
			...
			await page.type(
				type === 'expense' ? EXPENSE_AMOUNT_INPUT_SELECTOR : INCOME_AMOUNT_INPUT_SELECTOR,
				amount.toString()
			);

			await page.evaluate(async (CATEGORY_DROPDOWN_SELECTOR) => {
				const dropdown = document.querySelector(CATEGORY_DROPDOWN_SELECTOR) as HTMLDivElement;
				if (!dropdown) return;
				dropdown.click();
