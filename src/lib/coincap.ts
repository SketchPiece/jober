import createServiceError from './helpers/createServiceError';

const CMC_BALANCE_URI = 'https://api.coinmarketcap.com/asset/v4/portfolio/group/queryAll';
...
export class CoinCapClient {
	...
	async getPortfolioBalance(portfolioName: string) {
		try {
			const responce = await fetch(CMC_BALANCE_URI, {
				headers: {
			...
			const portfolioData = responce.data.find(
				(portfolio: { portfolioName: string }) => portfolio.portfolioName === portfolioName
			);
